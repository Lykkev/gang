...
module.exports = bot => {
  ...
  bot.on("channelCreate", async channel => {
    ...
    if (logs) {
      ...
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} creado un canal.`;
  ...
  bot.on("channelDelete", async channel => {
    ...
    if (logs) {
      ...
      // Adds to the embed
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} eliminado #${channel.name}.`;
        embed.author.icon_url = user.avatarURL;
      }
    }
  ...
  bot.on("channelUpdate", async (channel, oldchannel) => {
    ...
    if (logs) {
      ...
      const user = logs.users[0];
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} editado #${oldchannel.name}.`;
        embed.author.icon_url = user.avatarURL;
      }
    }
