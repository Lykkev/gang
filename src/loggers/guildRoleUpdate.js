...
module.exports = bot => {
  ...
  bot.on("guildRoleCreate", async (guild, role) => {
    ...
    if (logs) {
      ...
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} a creado un rol.`;
  ...
  bot.on("guildRoleDelete", async (guild, role) => {
    ...
    if (logs) {
      ...
      // Adds to the embed
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} eliminado @${role.name}.`;
        embed.author.icon_url = user.avatarURL;
      }
    }
  ...
  bot.on("guildRoleUpdate", async (guild, role, oldrole) => {
    ...
    if (role.hoist !== undefined && oldrole.hoist !== undefined && role.hoist !== oldrole.hoist) {
      embed.fields.push({
        name: "Visibilidad",
        value: role.hoist ? "No se muestra por separado ➜ Se muestra por separado" : "Se muestran por separado ➜ No se muestran por separado",
      });
    }
    ...
    if (role.mentionable !== oldrole.mentionable) {
      embed.fields.push({
        name: "Mencionabilidad",
        value: role.mentionable ? "Innombrable ➜ Mencionable" : "Mencionable ➜ Innombrable",
      });
    }
    ...
    if (logs) {
      ...
      // Adds to the embed
      if (log && new Date().getTime() - new Date(log.id / 4194304 + 1420070400000).getTime() < 3000) {
        embed.author.name = `${bot.tag(user)} actualizado @${role.name}.`;
        embed.author.icon_url = user.avatarURL;
      }
    }
