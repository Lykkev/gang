...
module.exports = async (msg, bot, cfg) => {
  ...
  if (/(https?:\/\/)?(www\.)?(discord\.(gg|io|me|li|list)|discord(app)?\.com\/invite)\/.+[a-z]/.test(msg.content)) {
    ...
    cfg.invitePunishments.forEach(async punishment => {
      if (punishment === "Warn") warning = await punish.warn(msg, bot, "Envió una invitación (Automod)");
    ...
    if (cfg.msgOnPunishment) {
      const pmsg = await msg.channel.createMessage(bot.embed("⚠️ Automod", "No se permiten invitaciones aquí.", "error"));
      setTimeout(() => pmsg.delete("AutoMod borró el mensaje").catch(() => {}), 4000);
    }
