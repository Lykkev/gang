...
module.exports.commands = async function loadCommands(bot) {
  ...
  files.forEach(subfolder => {
    ...
    cmds.forEach(cmd => {
      ...
      try {
        ...
        bot.log(`${cmd} falló al cargar: ${err}`);
      }
  ...
  bot.log.info(`${bot.commands.length} comandos cargados`);
};
...
module.exports.events = async function loadEvents(bot) {
  ...
  files.forEach(evnt => {
    ...
    try {
      ...
      bot.log(`${evnt} falló al cargar: ${err}`);
    }
  ...
  bot.log.info(`${bot.events.length} eventos cargados`);
};
...
module.exports.loggers = async function loadLoggers(bot) {
  ...
  files.forEach(l => {
    ...
    try {
      ...
      bot.log(`Logger ${l} fallo al cargar: ${err}`);
    }
  ...
  bot.log.info(`${bot.loggers.length} loggers cargados`);
};
  ...
  if (process.uptime() < 20) {
    ...
    files.forEach(w => {
      ...
      try {
        ...
        bot.log(`Webserver ${w} fallo al cargar: ${err}`);
      }
    });
  }
};
