...
class xkcdCommand extends Command {
  constructor(...args) {
    super(...args, {
      args: "[id:string]",
      description: "Envía un cómic desde XKCD.",
      allowdms: true,
      cooldown: 3,
    });
  }
  ...
  async run(msg, args) {
    ...
    if (!body || !body.num) return this.bot.embed("❌ Error", "Comic no encontrado.", msg, "error");
    ...
    msg.channel.createMessage({
      embed: {
        ...
        footer: {
          ...
          text: `Pedido por ${this.bot.tag(msg.author)} | Publicado en ${body.day}/${body.month}/${body.year}`,
        },
      },
    });
  }
